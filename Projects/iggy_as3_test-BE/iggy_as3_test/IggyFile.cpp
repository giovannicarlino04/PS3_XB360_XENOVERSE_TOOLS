...
bool IggyFile::LoadFlashData32(FixedMemoryStream &stream)
{
    ...
    hdr.main_offset = static_cast<uint16_t>(hdr.main_offset);
    hdr.as3_section_offset = static_cast<uint16_t>(hdr.as3_section_offset);
    hdr.as3_names_offset = static_cast<uint16_t>(hdr.as3_names_offset);
    hdr.as3_code_offset = static_cast<uint16_t>(hdr.as3_code_offset);
    hdr.names_offset = static_cast<uint16_t>(hdr.names_offset);
    hdr.last_section_offset = static_cast<uint16_t>(hdr.last_section_offset);
    hdr.unk_offset = BE32(hdr.unk_offset);
    hdr.unk_offset2 = BE32(hdr.unk_offset2);
    hdr.unk_offset3 = BE32(hdr.unk_offset3);    
    hdr.unk_offset4 = BE32(hdr.unk_offset4);
    hdr.unk_offset5 = BE32(hdr.unk_offset5);
    hdr.unk_offset6 = BE32(hdr.unk_offset6);


    DPRINTF("%zu\n%zu\n%zu\n%zu\n%zu\n%zu\n", hdr.main_offset, hdr.as3_section_offset, hdr.as3_names_offset, hdr.names_offset, hdr.last_section_offset);

    //size = BE32(size);

    // Check if any absolute offset exceeds the size of the stream
    ...
    // Read main section
    main_section.resize(hdr.as3_section_offset);
    if (!stream.Read(main_section.data(), main_section.size()))
    {
        DPRINTF("%s: Failure at main_section.", FUNCNAME);
        return false;
    }

    // Read AS3 names section
    size_t next_offset = (hdr.as3_code_offset != 1) ? hdr.as3_code_offset : ((hdr.names_offset != 1) ? hdr.names_offset : hdr.last_section_offset);
    as3_names_section.resize(next_offset - hdr.as3_names_offset);
    stream.Seek(hdr.as3_names_offset, SEEK_SET);
    if (!stream.Read(as3_names_section.data(), as3_names_section.size()))
    {
        DPRINTF("%s: Failure at as3_names_section.", FUNCNAME);
        return false;
    }
    ...
    {
        size_t as3_code_size = (hdr.names_offset != 1) ? hdr.names_offset : hdr.last_section_offset;
        size_t as3_code_offset = next_offset;

        as3_code_section.resize(as3_code_size - as3_code_offset);
        stream.Seek(as3_code_offset, SEEK_SET);
        if (!stream.Read(as3_code_section.data(), as3_code_section.size()))
        {
            DPRINTF("%s: Failure at as3_code_section.", FUNCNAME);
            return false;
        }
    }
    ...
    {
        size_t names_size = (hdr.last_section_offset != 1) ? hdr.last_section_offset : size;
        size_t names_offset = (hdr.as3_code_offset != 1) ? hdr.as3_code_offset : ((hdr.as3_names_offset != 1) ? hdr.as3_names_offset : next_offset);

        names_section.resize(names_size - names_offset);
        stream.Seek(names_offset, SEEK_SET);
        if (!stream.Read(names_section.data(), names_section.size()))
        {
            DPRINTF("%s: Failure at names_section.", FUNCNAME);
            return false;
        }
        DPRINTF("names_offset: %zu, names_size: %zu\n", names_offset, names_size);
    }

    // Read last section
    if (hdr.last_section_offset != 1)
    {
        size_t last_section_offset = (hdr.names_offset != 1) ? hdr.names_offset : next_offset;

        last_section.resize(size - last_section_offset);
        stream.Seek(last_section_offset, SEEK_SET);
        if (!stream.Read(last_section.data(), last_section.size()))
        {
            DPRINTF("%s: Failure at last_section.", FUNCNAME);
            return false;
        }
               DPRINTF("last_section_offset: %zu, last_section_size: %zu\n", last_section_offset, size - last_section_offset);
 }


    *(uint16_t *)&as3_code_section[4] = BE16(*(uint16_t *)&as3_code_section[4]);

    assert(stream.Tell() == size);
    // Check AS3 code section size
    if (!as3_code_section.empty())
    {
        DPRINTF("AS3 code section size: %zu\n", as3_code_section.size());
        if (as3_code_section.size() < 8 ||  *(uint16_t *)&as3_code_section[4]  > as3_code_section.size() - 8)
        {
            DPRINTF("%s: Failed at AS3 size.\n", FUNCNAME);
            DPRINTF("Value at offset 4: %u\n", *(uint16_t *)&as3_code_section[4] );
            DPRINTF("Size - 8: %zu\n", as3_code_section.size() - 8);
            return false;
        }
    }
...
bool IggyFile::LoadFlashData64(FixedMemoryStream &stream)
{
    ...
    if (max_as3_size != 0)
    {
        if (max_as3_size < 0x10 || *(uint16_t *)&as3_ptr[8] > max_as3_size-0xC)
        {
            DPRINTF("%s: Failed at AS3 size.\n", FUNCNAME);
            return false;
        }
...
bool IggyFile::Load(const uint8_t *buf, size_t size)
{
    ...
    if (hdr->plattform[0] != 2 || hdr->plattform[2] != 2)
    {
...
uint8_t *IggyFile::GetAbcBlob(uint16_t *psize) const
{
    if (as3_code_section.empty())
    {
        DPRINTF("No AS3 code section found.\n");
        return nullptr;
    }

    const uint8_t *ptr = as3_code_section.data();
    *psize = (is_64) ? *(uint16_t *)&ptr[8] : *(uint16_t *)&ptr[4];
...
bool IggyFile::SetAbcBlob(void *buf, uint32_t size)
{
    ...
    {
        if (is_64)
            *(uint16_t *)&ptr[new_size-0x10] = 1;
        else
            *(uint16_t *)&ptr[new_size-8] = 1;
    }
